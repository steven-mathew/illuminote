...
class Player(pg.sprite.Sprite):
    ...
    def __init__(self, renderer, x, y, block_list, enemy=None):
        ...
        Bullet.sound_hit = pg.mixer.Sound('resources/place.wav')
        Bullet.sound_hit.set_volume(0.1)
    ...
    def update(self):
        ...
        for block in self.block_list:
            if self.rect.colliderect(block.rect) and block.block_type == 'portal' and self.tp_cooldown == 0:
                ...
                for _ in range(30):
                    sparks.append([[self.position[0], self.position[1]], random.randint(int(angle) - 45,int(angle) + 45), random.randint(
                            7, 10) / 10 * 3, 9 * random.randint(5, 10) / 10 * 1 * (-1 if facing_up or facing_down else 1), (255, 0, 0)])
                # if self.renderer.night:
                    # self.renderer.filter.blit(
                        # LIGHT_IMAGE, (self.position[0] - LIGHT_IMAGE.get_width() / 2, self.position[1] - LIGHT_IMAGE.get_height() / 2))
